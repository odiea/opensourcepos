...
function get_customer_manage_table_headers()
{
	...
	$headers = array(
		...
		array('email' => $CI->lang->line('common_email')),
		array('phone_number' => $CI->lang->line('common_phone_number')),
		array('points' => $CI->lang->line('customers_available_points_value')), 
...
function get_customer_data_row($person, $stats)
{
	...
	return array (
		...
		'email' => empty($person->email) ? '' : mailto($person->email, $person->email),
		'phone_number' => $person->phone_number,
		'points' => to_currency($person->points),
...
function get_cashups_manage_table_headers()
{
	...
	$headers = array(
		...
		array('closed_amount_giftcard' => $CI->lang->line('cashups_closed_amount_giftcard')),
...
function get_cash_up_data_row($cash_up)
{
	...
	return array (
    	...
    	'closed_amount_giftcard' => to_currency($cash_up->closed_amount_giftcard),
