	...
	<?php echo form_open($controller_name."/add", array('id'=>'add_item_form', 'class'=>'form-horizontal panel panel-default')); ?>
		<div class="panel-body form-group">
			<ul>
				...
				<?php if($this->Employee->has_grant('items', $this->session->userdata('person_id')))
				{
				?>
				<li class="pull-right">
...
function check_payment_type()
{
	...
	$("#add_payment_button_1").click(function()
	{
		var cash_rounding = <?php echo json_encode($cash_rounding); ?>;

		if($("#payment_types").val() == "<?php echo $this->lang->line('sales_cash'); ?>" && cash_rounding)
	{		
		$("#amount_tendered:enabled").val('<?php echo to_currency_no_money($quick_cash_1); ?>');			
		$('#add_payment_form').submit();
	}
	});
	
	$("#add_payment_button_2").click(function()
	{
		var cash_rounding = <?php echo json_encode($cash_rounding); ?>;

		if($("#payment_types").val() == "<?php echo $this->lang->line('sales_cash'); ?>" && cash_rounding)
	{		
		$("#amount_tendered:enabled").val('<?php echo to_currency_no_money($quick_cash_2); ?>');			
		$('#add_payment_form').submit();
	}
	});
	
	$("#add_payment_button_3").click(function()
	{
		var cash_rounding = <?php echo json_encode($cash_rounding); ?>;

		if($("#payment_types").val() == "<?php echo $this->lang->line('sales_cash'); ?>" && cash_rounding)
	{		
		$("#amount_tendered:enabled").val('<?php echo to_currency_no_money($quick_cash_3); ?>');			
		$('#add_payment_form').submit();
	}
	});
	
	$("#add_payment_button_4").click(function()
	{
		var cash_rounding = <?php echo json_encode($cash_rounding); ?>;

		if($("#payment_types").val() == "<?php echo $this->lang->line('sales_cash'); ?>" && cash_rounding)
	{		
		$("#amount_tendered:enabled").val('<?php echo to_currency_no_money($quick_cash_4); ?>');			
		$('#add_payment_form').submit();
	}
	});
	
	$("#add_payment_button_5").click(function()
	{
		var cash_rounding = <?php echo json_encode($cash_rounding); ?>;

		if($("#payment_types").val() == "<?php echo $this->lang->line('sales_cash'); ?>" && cash_rounding)
	{		
		$("#amount_tendered:enabled").val('<?php echo to_currency_no_money($quick_cash_5); ?>');			
		$('#add_payment_form').submit();
	}
	});
	
	$("#add_payment_button_6").click(function()
	{
		var cash_rounding = <?php echo json_encode($cash_rounding); ?>;

		if($("#payment_types").val() == "<?php echo $this->lang->line('sales_cash'); ?>" && cash_rounding)
	{		
		$("#amount_tendered:enabled").val('<?php echo to_currency_no_money($quick_cash_6); ?>');			
		$('#add_payment_form').submit();
	}
	});
}	
	$.getJSON( '<?php echo site_url($controller_name."/get_item_categories"); ?>', function(data)
 {
	   ...
	   $.each( data, function (key,val)
		{                
				...
				{
                   items.push("<li><button style='margin:2px;' type='button' class='btn btn-secondary' onclick='additem("+ikey+")'>"+ival+"</a></li>"); 
			  });
                items.push( "</ul>");
                items.push( "</div>");       
                console.log(items);				
